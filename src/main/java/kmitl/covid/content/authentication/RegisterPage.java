...
public class RegisterPage {
	public static GridPane getContent() {
		...
		RegisterPage.node = new GridPane(); // GridPane คือตารางเป็นช่องคล้าย ๆ Excel
		RegisterPage.node.setAlignment((Pos.TOP_LEFT)); // ตั้งค่าให้อะไรก็ตามที่อยู่ในช่องมันเรียงชิดซ้าย
		RegisterPage.node.setVgap(9); // ให้แต่ละช่องห่างกันแนวตั้ง 10 pixels
		RegisterPage.node.setHgap(50); // ให้แต่ละช่องห่างกันแนวนอน 50 pixels

		// สร้างข้อกำหนดทั้งหมด 6 คอลัมน์ โดยกำหนดให้มีขนาดตามนี้
		...
		//Username
		Label usernameLabel = new Label("Username"); //อารมณ์ประมาณสร้างป้ายข้อความ
		...
		RegisterPage.node.add(username, 1, 0); // เอาสิ่งที้่เราเขียนไป ยัดเข้าไปตัวหลัก (GridPage) ในตำแหน่งที่ 1,0 ของตาราง
		...
		GridPane.setColumnSpan(usernameField, 2); // เซตให้มัน Merge คอลัมน์ไป 2 ช่อง คือกินช่องทั้งหมด 2 ช่องนั่นเอง
		RegisterPage.node.add(usernameField, 1, 1); // ยัดใส่เช่นเดิมในตำแหน่งที่ 1,1

		//National ID
		Label nationalidLabel = new Label("National ID"); //อารมณ์ประมาณสร้างป้ายข้อความ
		...
		RegisterPage.node.add(nationalid, 1, 4);
		...
		RegisterPage.node.add(nationalidField, 1, 5); // ยัดใส่เช่นเดิมในตำแหน่งที่ 1,6

		//First Name
		Label firstnameLabel = new Label("First Name"); //อารมณ์ประมาณสร้างป้ายข้อความ
		...
		RegisterPage.node.add(firstname, 1, 8);
		...
		RegisterPage.node.add(firstnameField, 1, 9); // ยัดใส่เช่นเดิมในตำแหน่งที่ 1,11

		//Name Title
		Label nametitleLabel = new Label("Name Title"); //อารมณ์ประมาณสร้างป้ายข้อความ
		...
		RegisterPage.node.add(nametitle, 1, 12);
		...
		RegisterPage.node.add(nametitleField, 1, 13); // ยัดใส่เช่นเดิมในตำแหน่งที่ 1,11

		//E-mail
		Label emailLabel = new Label("E-mail"); //อารมณ์ประมาณสร้างป้ายข้อความ
		...
		RegisterPage.node.add(email, 1, 16);
		...
		RegisterPage.node.add(emailField, 1, 17); // ยัดใส่เช่นเดิมในตำแหน่งที่ 1,11

		//Address
		Label addressLabel = new Label("Address"); //อารมณ์ประมาณสร้างป้ายข้อความ
		...
		RegisterPage.node.add(address, 1, 20);

		//กล่อง Address
		TextField addressField = new TextField(); // สร้างช่องใส่รหัสบัตรปชช
		addressField.setFont(KornFont.paragraphNormal); // เซตฟอนเช่นเดิม
		addressField.setPromptText("อีเมล"); // ใส่ตัวหนังสือเทา ๆ บอกใบ้ข้างในช่องใส่ข้อความ
		GridPane.setColumnSpan(addressField, 2); // เซตให้มัน Merge คอลัมน์ไป 3 ช่อง คือกินช่องทั้งหมด 3 ช่องนั่นเอง
		RegisterPage.node.add(addressField, 1, 21); // ยัดใส่เช่นเดิมในตำแหน่งที่ 1,11

		///ขวา////////////////////////////////////////////////////////////////////////////////////////////////////////////

		//Password
		Label passwordLabel = new Label("Address"); //อารมณ์ประมาณสร้างป้ายข้อความ
		...
		RegisterPage.node.add(password, 3, 0);
		...
		passwordField.setPromptText("อีเมล"); // ใส่ตัวหนังสือเทา ๆ บอกใบ้ข้างในช่องใส่ข้อความ
		GridPane.setColumnSpan(passwordField, 2); // เซตให้มัน Merge คอลัมน์ไป 3 ช่อง คือกินช่องทั้งหมด 3 ช่องนั่นเอง
		RegisterPage.node.add(passwordField, 3, 1); // ยัดใส่เช่นเดิมในตำแหน่งที่ 1,11

		//Last Name
		Label lastnameLabel = new Label("Last Name"); //อารมณ์ประมาณสร้างป้ายข้อความ
		...
		RegisterPage.node.add(lastname, 3, 8);
		...
		RegisterPage.node.add(lastnameField, 3, 9); // ยัดใส่เช่นเดิมในตำแหน่งที่ 1,11

		//Gender
		Label genderLabel = new Label("Gender"); //อารมณ์ประมาณสร้างป้ายข้อความ
		...
		RegisterPage.node.add(gender, 2, 12);
		...
		RegisterPage.node.add(genderField, 2, 13); // ยัดใส่เช่นเดิมในตำแหน่งที่ 1,11

		//Birthdate
		Label birthdateLabel = new Label("Birthdate (DD/MM/YYYY)"); //อารมณ์ประมาณสร้างป้ายข้อความ
		...
		RegisterPage.node.add(birthdate, 3, 12);
		...
		RegisterPage.node.add(birthdateField, 3, 13); // ยัดใส่เช่นเดิมในตำแหน่งที่ 1,11

		//Phone number
		Label phonenumberLabel = new Label("Phone Number"); //อารมณ์ประมาณสร้างป้ายข้อความ
		...
		RegisterPage.node.add(phonenumber, 3, 16);
		...
		phonenumberField.setPromptText("นามสกุล"); // ใส่ตัวหนังสือเทา ๆ บอกใบ้ข้างในช่องใส่ข้อความ
		GridPane.setColumnSpan(phonenumberField, 2); // เซตให้มัน Merge คอลัมน์ไป 3 ช่อง คือกินช่องทั้งหมด 3 ช่องนั่นเอง
		RegisterPage.node.add(phonenumberField, 3, 17); // ยัดใส่เช่นเดิมในตำแหน่งที่ 1,11

////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

		//Save Button
		Button signUpButton = new Button("Sign Up"); // สร้างปุ่ม Save
		signUpButton.setFont(KornFont.paragraphNormal); // เซตฟอนต์เช่นเดิม
		signUpButton.setMinWidth(Column3.getPrefWidth()); // เซตขนาดความกว้างของปุ่มให้มันขนาดตามกับขนาดช่องของคอลัมน์นั้น ๆ คือทำให้เต็มช่องนั่นแหละ...
		signUpButton.setDefaultButton(true); // เซ็ตให้เป็นปุ่มเริ่มต้น หมายถึงถ้ามีปุ่มนี้ในหน้าไหน กด Enter จะเป็นเรียกการทำงานของปุ่ม
		...
		RegisterPage.node.add(signUpButton, 2, 28); // ยัดใส่เช่้นเดิม

		//Back Button
		Button backButton = new Button("Back"); // สร้างปุ่ม Save
		...
		backButton.setDefaultButton(true); // เซ็ตให้เป็นปุ่มเริ่มต้น หมายถึงถ้ามีปุ่มนี้ในหน้าไหน กด Enter จะเป็นเรียกการทำงานของปุ่ม
		//loginButton.setGraphic(KornIcon.getIconRegular("\uF2F6")); // ใส่ Icon หน้า Text ของปุ่ม
		//loginButton.setGraphicTextGap(10); // ระยะห่างระหว่าง Icon กับ Text
